"""
Transposed KPI Tracker — Rana (v5)
Matches KPI-Abid architecture exactly: no title row, correct per-section merges.
...
date_cols = sorted(
    [(ci, parse_date(header[ci]), header[ci].strip())
     for ci in range(3, min(55, len(header)))
     if parse_date(header[ci])],
    key=lambda x: x[1]
)
print(f"Rana dates: {date_cols[0][2]} → {date_cols[-1][2]}  ({len(date_cols)} cols)")

# ── Build rows (no title row — section headers start at row 1) ────────────────
SECTION_ROW = ['Date']
METRIC_ROW  = ['Date']
for s in SECTION_ORDER:
    n = NCOLS[s]
...
try:
    ...
    ws = sh.add_worksheet('KPI-Rana', rows=200, cols=25)

ws.update(range_name='A1', values=all_rows, value_input_option='USER_ENTERED')
print("Data written")

# ── Palette (exact from user-approved design) ─────────────────────────────────
def rgb(r, g, b):
...
# ── Freeze THEN merge (per-section only — no big merge) ───────────────────────
