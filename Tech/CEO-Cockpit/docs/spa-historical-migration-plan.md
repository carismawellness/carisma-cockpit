...
- Add the data-source rule to `Tech/CEO-Cockpit/AGENTS.md` (live ETL still ONLY pulls Cockpit Datasheet; historic table is a one-off backfill, not re-run nightly).
- Update `Tech/CEO-Cockpit/CLAUDE.md` memory note (`project_cockpit_etl.md`) to mention `spa_transactions_raw` exists.
...
- DDL: `Tech/CEO-Cockpit/supabase/migrations/070_create_spa_transactions_raw.sql`
- Backfill script: `Tech/CEO-Cockpit/scripts/spa-historical-backfill.ts`
- QC script: `Tech/CEO-Cockpit/scripts/qc-spa-historical.ts`
- Optional ETL API: `Tech/CEO-Cockpit/app/api/etl/spa-historical/route.ts`
