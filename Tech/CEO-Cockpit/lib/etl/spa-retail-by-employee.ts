...
// Target: spa_retail_by_employee_daily (month-scoped delete + insert).
// Mirrors lib/etl/spa-services-by-employee.ts.
//
// Sheet quirks (verified 2026-06-10):
//   • Row 1 is a banner ("Product data is from 1 Jan 2025") — headers are on
//     the SECOND row; the ≥3-non-empty-cells scan below finds them.
//   • Several headers carry a trailing space: "VAT Exclusive Amount ",
//     "Material ", "Point of Sales " — stripCol tolerates `${key} `.
//   • "Date" format is like "15 April 2026".
//   • "VAT Exclusive Amount" is already ex-VAT — stored as-is.
...
export async function runSpaRetailByEmployee(
  dateFrom: string,
  dateTo:   string,
): Promise<{ rowsInserted: number; log: string[] }> {
  // Observability wrapper — records start/success/fail to etl_sync_log
  // (log key "spa_retail_by_employee"). Data logic lives in the inner function.
  const logger = new ETLLogger("spa_retail_by_employee");
  await logger.start();
  try {
    const result = await runSpaRetailByEmployeeInner(dateFrom, dateTo);
...
async function runSpaRetailByEmployeeInner(
  dateFrom: string,
  dateTo:   string,
): Promise<{ rowsInserted: number; log: string[] }> {
  const log: string[] = [];
  const allRows = await fetchCockpitCsv();
  log.push(`Fetched ${allRows.length} raw rows from Cockpit "${COCKPIT_TABS.SPA_RETAIL.name}" tab`);
  ...
  for (const row of allRows) {
    const employee = stripCol(row, "Sales Employee");
    if (!employee) { skipEmployee++; continue; }

    const dateRaw = stripCol(row, "Date");
    ...
    const amountEx = safeFloat(stripCol(row, "VAT Exclusive Amount"));
    if (amountEx <= 0) { skipBadAmount++; continue; }

    const dateStr  = dateKey(d);
    const monthKey = `${dateStr.slice(0, 7)}-01`;
    if (!validMonths.has(monthKey)) { skipOutOfRange++; continue; }

    // Unknown Point of Sales → keep the row with location_id null so
    // employee attribution is never lost (column is nullable by design).
    const locId = SPA_LOC_MAP[stripCol(row, "Point of Sales")] ?? null;
    ...
    buckets.get(monthKey)!.push({
      ...
      product_name:  stripCol(row, "Material") || null,
      product_brand: stripCol(row, "Brand")    || null,
      amount_ex_vat: +amountEx.toFixed(2),
    });
  }
  ...
  const badDateSuffix = badDateSamples.length
    ...
    : "";
  log.push(
    `Skips — empty_employee:${skipEmployee} bad_date:${skipBadDate}${badDateSuffix} ` +
    `bad_amount:${skipBadAmount} out_of_range:${skipOutOfRange}`,
  );

  let totalRows = 0;
  for (const [monthKey, rows] of buckets) {
    await deleteWhere("spa_retail_by_employee_daily", { month: monthKey });
    const n = await insertRows("spa_retail_by_employee_daily", rows);
    totalRows += n;
    const exTotal = rows.reduce((s, r) => s + Number(r.amount_ex_vat), 0);
