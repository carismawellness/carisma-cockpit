...
type TalexioAction = "employees" | "headcount" | "timelogs" | "leave" | "shifts" | "shiftsToday" | "payrolls" | "payslips" | "sickLeave";
...
// ---- Sick leave types and hook ----

export interface TalexioLeaveRequest {
  id: string;
  from: string;
  to: string;
  numberOfDays: number;
...
export interface TalexioEmployeeWithLeaveRequests {
  id: string;
  fullName: string;
  isTerminated: boolean;
  leaveRequests: TalexioLeaveRequest[];
}

/** Fetches approved leave requests for all active employees within [from, to] (YYYY-MM-DD). */
export function useTalexioSickLeave(from: string, to: string) {
  return useTalexio<{ employees: TalexioEmployeeWithLeaveRequests[] }>({
    action: "sickLeave",
    params: { dateFrom: from, dateTo: to },
    enabled: !!from && !!to,
  });
