...
There are **16 ETL pipelines** to build, plus **3 missing data sources** that need new tables and pipelines. Python ETL scripts already exist in the codebase (at `10-Tech/CEO-Cockpit/etl/`) — you can reference their logic for column mappings and transformations, but the goal is to rebuild these as N8N workflows where practical. For complex Google Sheets parsing with specific row/column logic, consider using N8N's Execute Command node to call the existing Python scripts instead of recreating that logic in N8N Code nodes.
